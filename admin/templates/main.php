...
$menu_tab = RoleCraftRequest::get( 'tab', $nonce );
?>
<div class="wpss-roles-admin-container">
	<h3>
		...
		if ( ! empty( $menu_tab ) ) :
			$section_title = WPSSAdminFrontend::nav_menu_tabs()[ $menu_tab ];
		endif;
	...
	<div class="content-nav">
		<ul class="admin-tabs">
			...
			foreach ( WPSSAdminFrontend::nav_menu_tabs() as $tab => $title ) :
				++$current_tab;
				$active_tab = 1 === $current_tab && empty( $menu_tab ) ? 'active' : '';
				if ( ! empty( $menu_tab ) && $tab === $menu_tab ) :
					$active_tab = 'active';
				endif;
		...
		<div class="tab-content">
			<?php
			if ( ! empty( $menu_tab ) && in_array( $menu_tab, WPSSAdminFrontend::template_whitelist(), true ) ) :
				WPSSAdminFrontend::render( [ 'template' => $menu_tab ] );
			else :
