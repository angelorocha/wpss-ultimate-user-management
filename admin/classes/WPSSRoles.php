<?php
/**
 * Manages custom WordPress roles: add, remove, and list.
 *
...
class WPSSRoles {
	...
	/**
	 * Register the AJAX actions to add and remove roles.
	 *
	 * @since 1.0.0
	 */
	...
	#[NoReturn] public function add_role_action(): void {
		...
		if ( ! is_null( $add_role_action ) ) {
			...
			printf( esc_html__( 'Role %s successfully added!', 'wpss-ultimate-user-management' ), esc_html( $add_role_action['name'] ) );
		} else {
	...
	#[NoReturn] public function remove_role_action(): void {
		...
		if ( ! in_array( $role_id, self::$roles_filter, true ) ) {
			...
			if ( $remove_action ) {
				$nonce         = wp_create_nonce( WPSSUserRolesCapsManager::nonce() );
				$get_role_name = RoleCraftRequest::post( 'role_name', false, $nonce );
				/* Translators: %s is a role name */
				printf( esc_html__( 'Role %s successfully removed!', 'wpss-ultimate-user-management' ), esc_html( $get_role_name ) );
			} else {
	...
	public static function get_roles_names( bool $filter = true ): array {
		...
		if ( $filter ) {
			...
			return array_filter(
				$roles,
				function ( $role ) use ( $filter ) {
					if ( in_array( $role, $filter, true ) ) {
						return false;
					}
	 ...
	 *
	 * @param string $role Role slug being removed.
	 *
	 * @since 1.0.0
	 */
	 ...
	 *
	 * @param string $role Human-readable role name.
	 *
	 * @return array|null
	 * @since 1.0.0
	 */
	...
	public function remove_role( string $role ): bool {
		if ( in_array( $role, array_keys( self::instance()->get_roles_names() ), true ) ) {
			remove_role( $role );
